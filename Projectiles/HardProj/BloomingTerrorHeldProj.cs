...
{
    public class BloomingTerrorHeldProj : KnifeProjectile
    {
        ...
        float ChargeCounter;
        float ChargeDivisionFactor = 5f;
        static int ShootTimerMax = 13;
        int ShootTimer;
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                if (stillInUse)
                {
                    ...
                    if (ShootTimer > ShootTimerMax)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), InitialPos.X, InitialPos.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ProjectileType<BloomingTerrorProj>(), Projectile.damage, Projectile.knockBack, player.whoAmI);
        ...
        private void UpdatePlayerVisuals(Player player, Vector2 playerHandPos)
        {
            ...
            {
                Projectile.rotation = Projectile.velocity.ToRotation() + (MathHelper.PiOver2 * Projectile.direction);
            }
            else
            {
                Projectile.rotation = Projectile.velocity.ToRotation() - ((MathHelper.Pi-MathHelper.PiOver2) * Projectile.direction);
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(ModContent.Request<Texture2D>("VKE/Projectiles/HardProj/BloomingTerrorHeldProj_Glow").Value, sheetInsertPosition, new Rectangle?(new Rectangle(0, spriteSheetOffset, texture.Width, frameHeight)), glowColor, Projectile.rotation, new Vector2(texture.Width / 2f, frameHeight / 2f), Projectile.scale, effects, 0);
            return false;
        }
    }
}