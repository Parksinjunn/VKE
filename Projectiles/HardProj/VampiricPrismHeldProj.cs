            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                if (stillInUse)
                {
                    ...
                    if (ShootTimer > ShootTimerMax)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), InitialPos.X, InitialPos.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ProjectileType<VampiricPrismProj>(), 1 + (int)((float)Projectile.damage * ((float)ShootTimerAdd /4f)), Projectile.knockBack, player.whoAmI);
