...
{
    public class EverscreamsBranchHeldProj : KnifeProjectile
    {
        ...
        //public const float DamageStart = ;
        ...
        public override void AI()
        {
            ...
            HealthTimer += 1;
            if(HealthTimer > 20 && player.statMana >= 8)
            {
                HealthTimer = 0;
                player.statMana-= 8;
                player.manaRegenDelay = 20;
            }
            else if(player.statMana < 8)
            {
                Projectile.Kill();
            }
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                if (stillInUse)
                {
                    ...
                    int ShootTimerAdd = (int)(1f * (450 / (MaxCharge / ChargeDivisionFactor + numProjectiles2)));
                    int SpreadAdd = (int)(1f * (5 / (MaxCharge / ChargeDivisionFactor + numProjectiles2)));
                    if (ShootTimerAdd < 1)
                    {
                        ShootTimer++;
                    }
                    ...
                    if (ShootTimer > ShootTimerMax)
                    {
                        ...
                        Vector2 InitialPos = (player.RotatedRelativePoint(player.MountedCenter, true)) + (Projectile.velocity * 3);
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), InitialPos.X, InitialPos.Y, baseSpeed * 1.6f * (float)Math.Sin(offsetAngle), baseSpeed * 1.6f * (float)Math.Cos(offsetAngle), ProjectileType<EverscreamsBranchProj>(), Projectile.damage, Projectile.knockBack, player.whoAmI);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                Offset = new Vector2(player.direction * 9, 0);
            }
            else
            {
                Offset = new Vector2(player.direction * 8, 0);
            }
            Vector2 sheetInsertPosition = ((player.RotatedRelativePoint(player.MountedCenter, true)) - Offset) + (Projectile.velocity * 2.6f) - Main.screenPosition;
